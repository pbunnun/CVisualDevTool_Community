...
void
NodeGraphicsObject::
set_embedQWidgetSize(QSize widgetSize)
{
...
mouseMoveEvent(QGraphicsSceneMouseEvent * event)
{
  ...
  {
    auto check_rapid_mouse_move = event->pos() - event->lastPos();
    auto diff = event->pos() - _pressMousePos;
    if( check_rapid_mouse_move.manhattanLength() > 30 )
      state.setResizing(NodeState::RAPID_MOUSE_MOVING);
    if (auto w = _node.nodeDataModel()->embeddedWidget() )
    {
      prepareGeometryChange();

      auto oldSize = _pressEmbeddedWidgetSize;

      oldSize += QSize(diff.x(), diff.y());
      if( oldSize.width() < geom.minEmbeddedWidgetSize().x() )
        oldSize.setWidth( geom.minEmbeddedWidgetSize().x() );
      if( oldSize.height() < geom.minEmbeddedWidgetSize().y() )
        oldSize.setHeight( geom.minEmbeddedWidgetSize().y() );
...
mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
  ...
  if( state.resizing() == NodeState::RAPID_MOUSE_MOVING )
  {
    if( auto w = _node.nodeDataModel()->embeddedWidget() )
    {
      prepareGeometryChange();
      ...
      auto & geom = _node.nodeGeometry();
      auto embeddedWidgetSize = w->size();

      _proxyWidget->setMinimumSize(embeddedWidgetSize);
      _proxyWidget->setMaximumSize(embeddedWidgetSize);
      _proxyWidget->setPos(geom.widgetPosition());
